             ...
             var if_exist = await ifAccountExist();
                if(if_exist){ /// if user exist
                    ...
                    var hash_pass = await hashPass(hashed_pass)
                            ...
                            var template = await changeForgotPassworddAdmin(name,hashed_pass)
                            sendEmail(company_email, email, "Admin Forgot Password Request", template);
